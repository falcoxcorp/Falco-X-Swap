...
const CreateLaunchpad: React.FC = () => {
  const { t } = useTranslation();
  const [currentStep, setCurrentStep] = useState(1);
  const [formData, setFormData] = useState<LaunchpadDetails>({
    tokenAddress: '',
    tokenName: '',
    decimals: '18',
    mainFee: '5%',
  ...
  const handleInputChange = (field: keyof LaunchpadDetails, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
      ...
      <div>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2">
            <input
              type="radio"
              checked={formData.router === 'falcox'}
              onChange={() => handleInputChange('router', 'falcox')}
              className="text-yellow-500 focus:ring-yellow-500 bg-gray-800 border-gray-700"
            />
            <span className="text-sm text-white">Auto Listing</span>
          </label>
          <label className="flex items-center gap-2">
            <input
              type="radio"
              checked={formData.router === 'manual'}
              onChange={() => handleInputChange('router', 'manual')}
              className="text-yellow-500 focus:ring-yellow-500 bg-gray-800 border-gray-700"
            />
            <span className="text-sm text-white">Manual Listing</span>
          </label>
        </div>
        <p className="text-xs text-gray-500 mt-1">For auto listing, after you finalize the pool your token will be auto listed on DEX.</p>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            className="w-full bg-gray-800 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500"
          />
          <p className="text-xs text-gray-500 mt-1">SoftCap must be &gt;= 25% of HardCap!</p>
        </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2">
            <input
              type="radio"
              checked={formData.refundType === 'refund'}
              onChange={() => handleInputChange('refundType', 'refund')}
              className="text-yellow-500 focus:ring-yellow-500 bg-gray-800 border-gray-700"
            />
            <span className="text-sm text-white">Refund</span>
          </label>
          <label className="flex items-center gap-2">
            <input
              type="radio"
              checked={formData.refundType === 'burn'}
              onChange={() => handleInputChange('refundType', 'burn')}
              className="text-yellow-500 focus:ring-yellow-500 bg-gray-800 border-gray-700"
            />
            <span className="text-sm text-white">Burn</span>
          </label>
        </div>
      </div>

      {/* Listing Rate and Liquidity */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm text-gray-400 mb-2">
            Listing rate *
      ...
      <div>
        ...
        <select
          ...
          className="w-full bg-gray-800 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 appearance-none"
        >
          <option value="falcox">FalcoX Swap</option>
        </select>
      </div>
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div>
          <label className="block text-sm text-gray-400 mb-2">Whitelist Link</label>
          <input
            type="url"
            value={formData.whitelistLink}
            onChange={(e) => handleInputChange('whitelistLink', e.target.value)}
  ...
  const handleSubmit = () => {
    ...
    console.log('Submitting launchpad:', formData);
  };
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-xl sm:text-2xl font-bold text-white">Create Launchpad</h1>
          <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-xs">Coming Soon</span>
        </div>
        <p className="text-sm sm:text-base text-gray-400">Launch your token with our launchpad platform</p>
      </div>
